...
- [x] 使用 `Files.createTempFile`，不要直接以原始檔名組合本機路徑。
- [x] 驗證圖片 MIME type、副檔名、檔案大小與空檔案。
- [x] 在 `finally` 中清理暫存檔，確保上傳失敗時也不殘留檔案。
- [x] 更新商品未上傳新圖片時，保留原本的 `imgUrl` 與 `imgName`。
- [x] 刪除商品圖片失敗時提供 retry／補償機制。
